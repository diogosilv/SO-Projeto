...
int main(int argc, char* argv[]) {
    ...
    //Formato: ./main1_ficheiros -n N   //onde N é o numero filhos a serem criados
    ...
    char * pathFicheiro = (char *) malloc(sizeof(char) * 40);
    ...
    for(int i = 0; i < numFilhos; i++){
	    ...
	    if(pids[i] == 0){   //CODIGO FILHOS
            sprintf(pathFicheiro, "result/mutations_files_%d.txt", i+1);
            ...
            while (g != NULL) {
                ...
                save_mutation_array(mutation_array, pathFicheiro, 0);
...
/**
 * Handler de sinais
 * @param signum - Numero do sinal
 * Faz regra de 3 simples para calcular percentagem de genomas comparados, relativamente aos numero de genomas totais
 * No printf, "\e[s" guarda a posicao do cursor no terminal, e "\e[u" volta para essa ultima posicao guardada
 */
