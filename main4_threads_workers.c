...
int numThreads;
char * pathFicheiro = "result/mutations_threads.txt";
pthread_mutex_t mutex_genomas_cmp = PTHREAD_MUTEX_INITIALIZER;
...
int main(int argc, char* argv[]) {
    ...
    if(argc == 3 && strcmp(argv[1], "-n") == 0){
        ...
        printf("Format: \"./file -n x\" where x is the number of threads\n");
        return -1;
    }
    ...
    THREAD_ARGS thread_args[numThreads];
    for(int i = 0; i < numThreads; i++){
        
        thread_args[i].first_genome = i;
        thread_args[i].g = gl->phead;
	    if(pthread_create(&thread_id[i], NULL, &worker, &thread_args[i]) != 0) {
    ...
    if(DEBUG){
        printf("PERCENTAGEM FINAL: %d\n", percentagemExecucao);
        printf("GENOMAS COMPARADOS: %d\n", genomasComparados);
    }
    return 0;
}
...
void * print_state(){
    ...
    while(1){
        pthread_mutex_lock(&mutex_genomas_cmp);
        int nComparados = genomasComparados;
        pthread_mutex_unlock(&mutex_genomas_cmp);
...
void * worker(void * param){
    ...
    while (thread_args->g != NULL) {
        ...
        save_mutation_array_and_tid(mutation_array, pathFicheiro, 0, pthread_self());
        ...
        for(int j = 0; j < numThreads; j++){     //Para cada filho incrementar N genomas
            thread_args->g = thread_args->g->pnext;
